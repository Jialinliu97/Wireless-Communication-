...
L_frame=130; 
N_packet=4000;  
psk8Mod=comm.PSKModulator(8,'PhaseOffset',0); %without PhaseOffset for 8PSK
psk8Demod=comm.PSKDemodulator(8,'PhaseOffset',0);
b=3; % Set 3 for 8PSK
SNRdBs=[0:1:20]; sq2=sqrt(2); 
for iter=1:3 
   ...
   for i_SNR=1:length(SNRdBs) 
      ...
      for i_packet=1:N_packet 
         symbol_data=randi(L_frame*b,NT); 
         ...
         Z=0; 
         for i=1:NR 
            R(:,i) = sum(H(:,i).*X,2)/sq_NT + sigma*(randn(L_frame,1)+1i*randn(L_frame,1)); 
            Z = Z + R(:,i).*conj(H(:,i)); 
         end 
         for m=1:P 
